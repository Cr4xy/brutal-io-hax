(function() {
    (function(n, t, q) {
        var wsUrl; // ws url to current game server
        var botWs;
        var bots = {};
        function setupWs() {
            botWs = new WebSocket("ws://127.0.0.1:1337");
            botWs.onclose = () => setTimeout(setupWs, 1000);
            botWs.onopen = () => {
                if (wsUrl) {
                    botWs.send(JSON.stringify({kind: "url", url: wsUrl}));
                }
            }
            botWs.onmessage = (e) => {
        ...
        var clientBotEnabled = true;
        var targetPlayers = false;
        var avoidFlails = true;
        var avoidIntersections = true;
        const MODE_SUICIDE = 1;
        const MODE_FOLLOW_MOUSE = 2;
        const MODE_KILL_KING = 4;
        var mode = 1;
        
        var kingX, kingY;
        var mouseWorldX, mouseWorldY;
        window.addEventListener('keydown', e => {
            if(e.key == "g") clientBotEnabled = !clientBotEnabled;
            else if(e.key == "p") targetPlayers = !targetPlayers;
            else if(e.key == "k") {
                mode *= 2;
                if (mode > MODE_KILL_KING) {
                    mode = MODE_SUICIDE;
                }
            }
        });
            ...
            if ("undefined" == typeof freestar &&
                ...
                if (null != adsense && adsense) n.googletag && (n.googletag.cmd.push(function() {
                    Ka && (Ka = !1, setTimeout(function() {
                        Ka = !0
                    }, 12E4), n.googletag && n.googletag.pubads && n.googletag.pubads().refresh && n.googletag.pubads().refresh(n.ads))
                }), jb++);
                else {
                    q("#mpu-top").show();
                    var b = q("#ldr-top");
        ...
        function kb(b, f, k, g, d, e) {
            for (var h = 6, a = -1; 2 > a; a += 2) {
                ...
                for (var l = 0; l < h; l++) {
                    ...
                    var c = 20 * d; - 1 == a ? b.translate((50 + c) * e, 0) : b.translate((57 +
                                                                                           1.4 * c) * e, 0);
                    b.scale(.9 * e, .9 * e);
        ...
        var Na = "https:" == n.location.protocol,
            Db = n.location != n.parent.location ? t.referrer : t.location.href,
        ...
        var bb = function() {
            ...
            if (botWs && botWs.readyState == WebSocket.OPEN) {
                switch (mode) {
                    case MODE_SUICIDE:
                        if (myEntity) {
                            botWs.send(JSON.stringify({kind: "pos", x: myEntity.flailDstX, y: myEntity.flailDstY}));
                        }
                        break;
                    case MODE_KILL_KING:
                        botWs.send(JSON.stringify({kind: "pos", x: kingX, y: kingY}));
                        break;
                    case MODE_FOLLOW_MOUSE:
                        botWs.send(JSON.stringify({kind: "pos", x: mouseWorldX, y: mouseWorldY}));
                        if (shiftKey) {
                            J.throttle = false;
                        }
                        break;
            ...
            const minAvoidDist = 300;

            if (clientBotEnabled && myEntity) {
                ...
                for (var i in A) {
                    var e = A[i];
                    if (!e.nick || (targetPlayers && e.id !== myEntity.id)) {
                    ...
                    if (e.id !== myEntity.id && e.flailX) {
                        ...
                        if (avoidIntersections && e.attracting)
                        {
                            ...
                            if (distToIntersection < minAvoidDist && len < 1000) {
                        ...
                        if (avoidFlails && dist < minAvoidDist) {
                ...
                if (1) {
                    ...
                    for (var i in runAwayEnemies) {
                        ...
                        angle += Math.PI / 2;
                        for (var i = 0; i < (minAvoidDist - dist) / 10; i++) {
            ...
            { // Hack
                ...
                if (closestEnemyBall) {
                    ...
                    d.strokeStyle = "rgba(255, 0, 0, 0.2)";
                    y.setupContext(d);
                    d.beginPath();
                    d.moveTo(y.x, y.y);
                    d.lineTo(closestEnemyBall.flailDstX, closestEnemyBall.flailDstY);
                    d.closePath();
                    d.lineWidth = 6;
                    d.fill();
                    d.stroke();
                    d.beginPath();
                    d.arc(closestEnemyBall.flailDstX, closestEnemyBall.flailDstY, e.flailDstRadius, 0, Math.PI * 2);
                    d.closePath();
                    d.fill();
                    d.stroke();
                    d.restore();
                ...
                for (var i in runAwayIntersections) {
                    ...
                    d.beginPath();
                    d.moveTo(y.x, y.y);
                    d.lineTo(e.flailDstX + en.flailClosestX, e.flailDstY + en.flailClosestY);
                    d.closePath();
                    d.lineWidth = 6;
                    d.fill();
                    d.stroke();
                ...
                if (Object.keys(bots).length <= 10) {
                    for (var i in bots) {
                        var bot = bots[i];
                        d.save();
                        d.globalAlpha = 0.4;
                        d.fillStyle = "#34E8EB";
                        d.strokeStyle = "#34E8EB";
                        y.setupContext(d);
                        d.beginPath();
                        d.moveTo(y.x, y.y);
                        d.lineTo(bot.x, bot.y);
                        d.closePath();
                        d.lineWidth = 6;
                        d.fill();
                        d.stroke();
        ...
        n.clickPlay = function(b) {
            K ? hb() : (ha = !1, X = 1, n.localStorage.nick = b, Fb++, J.mouseMoved = !1, m.sendNick(b, Ra), Ra && (q("#continueTop").hide(), q("#continueBR").hide(), q("#continue").hide(), q("#nickInput").show(), q("#skinPanel").hide(), q("#howto").show(), q("#beta").show(), Ra = !1))
        };
        ...
        void 0 == pb && (pb = 1);
        var xb = n.navigator.userAgent; - 1 < xb.indexOf("MSIE ") || xb.indexOf("Trident/");
        ...
        ga.prototype = {
            ...
            setStroke: function(b) {
                this._stroke != b && (this._stroke = b, this._dirty = !0)
            },
        ...
        var Jb = function() {
            ...
            Kb = function() {
                ...
                b.keydown = function(b) {
                    67 ==
                        b.keyCode && W && wa ? setTimeout(function() {
                        gb()
                    }, 10) : W || (32 == b.keyCode ? K && m.sendClick(!0) : 188 != b.keyCode && 49 != b.keyCode && (222 == b.keyCode ? Za = !Za : 51 != b.keyCode && 52 != b.keyCode && 53 != b.keyCode && 54 != b.keyCode && (55 == b.keyCode && L ? ab = !ab : 56 == b.keyCode ? drawBackground = !drawBackground : 57 == b.keyCode ? $a = !$a : 48 != b.keyCode && (75 == b.keyCode ? L && m.leave() : 27 == b.keyCode ? (q("#overlay").show(), Ha(), W = !0, K || (w = null)) : 70 == b.keyCode ? (t.fullscreenElement || t.mozFullScreenElement || t.webkitFullscreenElement || t.msFullscreenElement ?
                        t.exitFullscreen ? t.exitFullscreen() : t.msExitFullscreen ? t.msExitFullscreen() : t.mozCancelFullScreen ? t.mozCancelFullScreen() : t.webkitExitFullscreen && t.webkitExitFullscreen() : t.documentElement.requestFullscreen ? t.documentElement.requestFullscreen() : t.documentElement.msRequestFullscreen ? t.documentElement.msRequestFullscreen() : t.documentElement.mozRequestFullScreen ? t.documentElement.mozRequestFullScreen() : t.documentElement.webkitRequestFullscreen && t.documentElement.webkitRequestFullscreen(Element.ALLOW_KEYBOARD_INPUT),
                ...
                b.mouseup = function(b) {
                    W || K && 0 == b.button && m.sendClick(!1)
                };
                b.mousemove = function(f) {
                    shiftKey = f.shiftKey;
                    W || (b.mouseMoved = !0, nb = f.clientX, ob = f.clientY)
                };
            ...
            Mb = function() {
                ...
                this.draw = function(v) {
                    if (0 < g && !ha) {
                        ...
                        p = .21 * canvas.height;
                        d || (p = .55 * canvas.height, e.setColor("#f90600"), e.setStrokeColor("#f90600"), e.setRoundedFrameStyle("#f90600"), h && (h.setColor("#f90600"), h.setStrokeColor("#f90600"), h.setRoundedFrameStyle("#FF9999"), h.setRoundedFrameOpacity(.1)));
                        var q = e.render();
                    ...
                    O && !ha && (0 < ra && sa || !sa || 0 < Y) && (null == ja && (I = parseInt(20 * s) + 1, P = new ga(I, Fa, !1, Fa), P.setValue(O), P.setUsingRoundedFrame(!0), P.setRoundedFrameStyle(y),
                                                                                  P.setShadowBlur(7), ja = P.render()), v.drawImage(ja, Q / 2 - ja.width / 2, 1.1 * ja.height * (oa ? 1 - Y / 300 : Y / 300) - ja.height));
                    b != s && (c = null, b = s);
                    a && !W && (p = .3 * canvas.height, I = l / 1E3 * 10, 1 < I && (I = 1), c || (c = new ga(25 * s, "#FF0000", !1, "#FF0000"), c.setValue(a), c.setHMargin(10), c.setVMargin(10), c.setUsingRoundedFrame(!1), c.setShadowBlur(6), r = c.render(), z = new ga(15 * s, "#FF0000", !1, "#FF0000"), z.setValue(warningSecondMessage), z.setHMargin(10), z.setVMargin(10), z.setUsingRoundedFrame(!1), z.setShadowBlur(6), G = z.render()), v.save(), v.translate(.5 *
                    canvas.width - r.width / 2, p), m = .2 * s, n = Math.sin(l / 100) * m, v.globalAlpha = I - m + n, v.drawImage(r, 0, 0), v.drawImage(G, r.width / 2 - G.width / 2, .8 * r.height), v.restore())
                };
                ...
                this.renderLeaderboard = function(a, c) {
                    ...
                    for (P = 0; P < g; P++)
                        if (z = v[P]) {
                            ...
                            var u = u + ka(z.nick),
                                I = 0;
                            a.fillStyle = N == z.id ? "rgba(0,255,255,1.0)" : "rgba(0,255,255,0.4)";
                            a.font = d;
                            r = a.measureText(u).width;
                            a.fillText(u, 6 + l, O + I + h / 2);
                            r = a.measureText(z.score).width;
                            a.fillText(z.score, n - l - r - 6, O + I + h / 2);
                            f && (a.save(), a.translate(6 + l + G + 10, O + I + h / 2 - 1), a.scale(.8, .8), p.frames.crown.draw(a),
                                  a.restore());
                            O += h + e
                        }! x && w && (a.fillStyle = "rgba(0,255,255,1.0)", u = fa + ". " + ka(w.nick), r = a.measureText(u).width, a.fillText(u, 6 + l, O + I + h / 2), r = a.measureText(ba).width, a.fillText(ba, n - l - r - 6, O + I + h / 2))
                };
                ...
                this.drawPowerup = function(a) {
                    ...
                    da = "RED FLAIL";
                    ia || (ia = new ga(17 * s, c, !1, b), ia.setValue("RED FLAIL"), ia.setStrokeWidth(5), ia.setUsingRoundedFrame(!1), $ = ia.render());
                    a.globalAlpha = Z;
            ...
            Nb = function() {
                function b(a, c, b, l, e, d, h, r) {
                    ...
                    d.moveTo(a, c - l);
                    for (i = 0; i < b; i++) g = a +
                        Math.cos(v) * l, k = c + Math.sin(v) * l, d.lineTo(g, k), v += x, g = a + Math.cos(v) * e, k = c + Math.sin(v) * e, d.lineTo(g, k), v += x;
                    d.lineTo(a, c - l);
                    d.closePath();
                ...
                this.update = function(a) {
                    if (this.beingDeleted) 3 != this.killReason && (this.flailX += .4 * (this.flailDstX - this.flailOrigX) / 3, this.flailY += .4 * (this.flailDstY - this.flailOrigY) / 3);
                    else {
                    ...
                    this.beingDeleted && (this.shipScale -= .1, 0 > this.shipScale && (this.id == N && (N = 0, w = null, B.hideCapacity(), B.hidePowerupBar(), B.clearWarningMessage()), delete A[this.id]));
                    this.invulnerable ? (u -= a, 0 >= u && (u = 250, x = (n = !n) ? 1 : .4)) : (v = this.redFlailDeployed ? this.dangerLowFreq ? .6 :
                                                                                                1 : 1, x = 1);
                ...
                this.drawCar = function(b, e) {
                    ...
                    this.beingDeleted && (b.scale(1.4, 1.4), p.frames.car_shock.draw(b));
                ...
                this.drawAfter = function(a) {
                    ...
                    a.moveTo(f, g);
                    for (var u, n, p, s = 0; s < z; s++) u = this.chainSegments[s], n = u.x, p = u.y, s + 1 < z && (n = this.chainSegments[s + 1].x, p = this.chainSegments[s + 1].y), s % 2 && a.bezierCurveTo(f, g, u.x, u.y, n, p), f = u.x, g = u.y;
                    ...
                    !this.attached && this.attracting && (k = !k, a.save(), a.beginPath(), a.fillStyle = r, a.strokeStyle = r, this.redFlail && (a.fillStyle = "#FF0000", a.strokeStyle = "#FF0000"), f = .5, k && (f = 1), a.arc(u.x, u.y, 10 * f, 0, 2 * Math.PI), a.closePath(), a.fill(), k && (a.globalAlpha = .5 * this.shipScale * x, a.lineWidth = 3, a.beginPath(), a.arc(u.x,
                    u.y, 15, 0, 2 * Math.PI), a.closePath(), a.stroke(), a.globalAlpha = .5 * this.shipScale * x, a.lineWidth = 2, a.beginPath(), a.arc(u.x, u.y, 20, 0, 2 * Math.PI), a.closePath(), a.stroke()), a.restore());
                    u = this.flailRadius;
                    var t;
                    ...
                    V && (a.scale(f, f), g = .2, this.redFlailDeployed && (g = .3), a.globalAlpha = g * this.shipScale * x, I.draw(a), a.globalAlpha =
                          1 * this.shipScale * x * v, a.scale(1 / f, 1 / f));
                    ...
                    a.restore();
                    this.redFlailDeployed || (a.save(), a.globalAlpha = this.shipScale * x, a.beginPath(), f = u - 6 - g, 0 > f && (f = .1), a.arc(this.flailX, this.flailY, f * this.shipScale, 0, 2 * Math.PI, !1), a.fillStyle = c, a.fill(), a.restore());
                    this.attached ||
                        N != this.id || (c = this.locatorValue / 1E3, 0 > c && (c = 0), l = u + 30 + 40 * c, a.beginPath(), a.globalAlpha = .2 * (1 - c), a.lineWidth = 4, a.strokeStyle = "#FFFFFF", a.moveTo(this.flailX - l, this.flailY + l), a.lineTo(this.flailX + l, this.flailY + l), a.lineTo(this.flailX + l, this.flailY - l), a.lineTo(this.flailX - l, this.flailY - l), a.closePath(), a.lineCap = "round", c = 2 * l, a.setLineDash([.15 * c, .7 * c, .3 * c, .7 * c, .3 * c, .7 * c, .3 * c, .7 * c, .3 * c]), a.stroke(), a.globalAlpha = 1, a.lineCap = "butt", a.lineWidth = 1, a.setLineDash([]))
                };
                ...
                this.drawInfo = function(a) {
                    a.save();
                    a.translate(this.x, this.y);
                    this.id == la && (a.translate(0, -60), p.frames.crown.draw(a), a.translate(0, 60));
                ...
                this.drawEnergy = function(a) {
                    ...
                    a.font = "Bold 15px 'proxima-nova-1','proxima-nova-2', arial, sans-serif";
                    a.textBaseline = "hanging";
                    var c = a.measureText(f.energy).width;
                    a.fillText(f.energy, -c / 2, 38);
                    a.restore()
                };
                ...
                this.updateChainFlail = function(a, c, b) {
                    ...
                    for (var e = 0; e < l; e++) {
                        ...
                        r.dstY = 10 * h;
                        b && (r.origX =
                              r.dstX, r.origY = r.dstY, r.x = r.dstX, r.y = r.dstY)
                    }
                    return c
                };
                this.updateNetworkFlail = function(a, c, b, l) {
                    ...
                    this.inside = v & 64;
                    this.charging = v & 128;
                    179 != l && (l = this.redFlail, this.redFlail = v & 256, !l && this.redFlail ? this.preRenderRedCar() : l && !this.redFlail && this.preRender(this.hue), l = this.redFlailDeployed, this.redFlailDeployed = v & 512, v = 0, this.redFlailDeployed && (v = a.getUint8(c, !0),
                    c++), !l && this.redFlailDeployed ? (this.flashFlailValue = 200, this.preRender(0), f && B.showPowerupBar(v)) : l && !this.redFlailDeployed && (this.flashFlailValue = 200, f && B.hidePowerupBar()), this.redFlailDeployed && f && B.setPowerupProgress(v));
                    a = !g && this.decay;
                    l = !k && this.still;
                    if (f) {
                ...
                this.deleteNetwork = function(a, c) {
                    ...
                    R && 0 != this.killReason ? (this.beingDeleted = !0, this.id == N && (3 == this.killReason ? (B.addMessage("ELECTROCUTED"), y.shake(), this.shock = !0) : 2 == this.killReason ? (B.addMessage("DELETED BY SENTINEL"), y.shake()) : 5 == this.killReason && (y.shake(), ha = !0))) :
                    (this.id == N && (N = 0, w = null, B.hideCapacity(), B.hidePowerupBar(), B.clearWarningMessage()), delete A[this.id]);
                    return c
                };
                ...
                this.preRenderRedCar = function() {
                    ...
                    I = p.frames.grid_mask_hard.renderTintedFrame("hsl(" +
                                                                  this.hue + ", 100%, 40%)")
                };
            ...
            Ob = function() {
                ...
                this.updateNetwork = function(b, d, a) {
                    ...
                    this.dstY = 10 * c;
                    this.dstAngle = r;
                    a ? (this.origX = this.dstX, this.origY = this.dstY, this.x = this.dstX, this.y = this.dstY, this.origAngle = this.dstAngle, this.hue = b.getUint16(d, !0), d += 2, f = "hsl(" + this.hue + ", 100%, 50%)", k = p.frames.energy.renderTintedFrame(f)) : this.canInterpolate = !0;
                    this.lastUpdateTime = D;
                    return d
                };
            ...
            Qb = function(b) {
                ...
                this.updateNetwork = function(l, c, r) {
                    ...
                    c += 1;
                    r && (this.origX = this.dstX, this.origY = this.dstY, this.x = this.dstX, this.y = this.dstY, this.origAngle =
                          this.dstAngle, this.positive = l.getUint8(c), c += 1, this.hue = this.positive ? 116 : 0, g = "hsl(" + this.hue + ", 100%, 50%)", d = p.frames.tri.renderTintedFrame(g), a.style = g, 3 == b && (e = p.frames.grid_mask.renderTintedFrame(g)));
                    this.lastUpdateTime = D;
                    return c
                };
                this.deleteNetwork = function(a, c) {
            ...
            Rb = function(b) {
                ...
                this.render = function(a, c, b) {
                    ...
                    var d;
                    c = 5;
                    0 == this.subType ? b ? (a.fillStyle = p.bgGridGreen, d = "#00FF00") : (a.fillStyle = "#003a08", d = "#00BB00", c = 2) : 1 == this.subType ? (a.fillStyle = "#023139", d = "#0bdfff") : 2 == this.subType ? (a.fillStyle = p.bgGridRed, d = "#FF0000") : 4 == this.subType ? (a.fillStyle = 1 == b ? p.bgCoreCharge : 0 == b ? p.bgCore : "#000000", c = 4, a.setLineDash([8, 11]), d = "#0bdfff", a.lineCap = "round") : 5 == this.subType && (a.fillStyle = "#000000", 1 == b ? (d = "#ffd200", a.setLineDash([20, 20]), c = 7) : 0 == b ? (d = "#00FF00", a.setLineDash([10, 10]), c = 5) : (d = "#000",
                    c = 0), a.lineCap = "round");
                    a.strokeStyle = d;
                ...
                this.drawObject = function(b) {
                    ...
                    if (u)
                        if (z || this.preRender(), 2 == this.subType && (a = (l = !l) ? .6 : 1), f)
                            if (1 == u.type) {
                                b.save();
                                var p = 0,
                                    m = 0;
                                ...
                                u = f;
                                5 == this.subType && (n += coreRotation, 1 == U ? 0 < n && (x += .05, .15 < x && (x = .15)) : 2 == U ? u = k : 4 == U && (b.scale(2.9, 2.9), d.draw(b), b.scale(1 / 2.9, 1 / 2.9), u = g), 1 != U && (x -= .05, 0 > x && (x = 0)), b.scale(1 + x, 1 + x), b.rotate(n));
                ...
                this.preRender = function() {
                    ...
                    if (a) {
                        ...
                        if (2 == a.type) {
                            ...
                            h = -x + this.margin;
                            this.render(c, !0, 0);
                            4 == this.subType && (k = t.createElement("canvas"), a = k.getContext("2d"), k.width = f.width, k.height = f.height, this.render(a, !0, 1), g = t.createElement("canvas"),
                                                  a = g.getContext("2d"), g.width = f.width, g.height = f.height, this.render(a, !0, 2))
                        } else 1 == a.type && (f.width = 2 * a.radius + 2 * this.margin, f.height = 2 * a.radius + 2 * this.margin, this.render(c, !0, 0), 0 == this.subType ? (k = t.createElement("canvas"), a = k.getContext("2d"), k.width = f.width, k.height = f.height, this.render(a, !0, 1)) : 5 == this.subType && (k = t.createElement("canvas"), a = k.getContext("2d"), k.width = f.width, k.height = f.height, this.render(a, !0, 1), g = t.createElement("canvas"), a = g.getContext("2d"), g.width = f.width, g.height = f.height,
                        this.render(a, !0, 2), d = p.frames.grid_mask_hard.renderTintedFrame("#9d00c6")));
                        z = !0
                    }
                };
                this.deleteNetwork = function(a, c) {
            ...
            Tb = function() {
                ...
                var b = 2 * Math.random() * Math.PI,
                    ...
                    k, g, d, e, h = 2 * Math.random() * Math.PI,
                    a = 0;
                this.canInterpolate = this.beingDeleted = !1;
                ...
                this.updateNetwork = function(a, b, l) {
                    ...
                    this.dstY = 10 * f;
                    this.dstAngle = x;
                    l ? (this.origX = this.dstX, this.origY = this.dstY, this.x = this.dstX, this.y = this.dstY, this.origAngle = this.dstAngle, this.hue = a.getUint16(b, !0), b += 2, k = "hsl(" + this.hue + ", 100%, 60%)", g = "hsl(" + this.hue + ", 100%, 30%)", d = p.frames.energy2.renderTintedFrame(k), p.frames.energy2.renderTintedFrame(g), e = p.frames.grid_mask.renderTintedFrame(k), this.type = a.getUint8(b), b += 1, this.preRenderParticle(this.type)) :
            ...
            Ub = function() {
                        ...
                        if (a = l.response) {
                            ...
                            if (191 != e) console.log("ERROR LOADING MAP FILE");
                                ...
                                for (console.log("Map Version: " + b.version);;) {
                                    ...
                                    e = a.getUint8(d, !0);
                                    d += 1;
                                    if (1 == h || 2 == h) h = a.getUint8(d, !0), d += 1, 1 == h ? d = b.loadCircle(a, d, e) : 2 == h && (d = b.loadPolygon(a, d, e))
                                }
                ...
                this.drawColliders = function(a) {
                    for (var b = this.objects.length, c = 0; c < b; c++) this.objects[c].draw(a, 1)
                };
                ...
                this.drawLimits = function(a) {
                    ...
                    d <= c[1].x && (a.fillStyle = "#023139", a.fillRect(d, -b / 2, 2500, b)); - d >= c[0].x && (a.fillStyle =
                                                                                                                "#023139", a.fillRect(-d - 2500, -b / 2, 2500, b)); - C / 2 > c[0].y && (a.fillStyle = "#023139", a.fillRect(-d - 2, -C / 2 - 2500, 2 * d + 4, 2500));
                    C / 2 < c[1].y && (a.fillStyle = "#023139", a.fillRect(-d - 2, C / 2, 2 * d + 4, 2500));
                    d <= c[1].x && a.drawImage(k, d - 30, -C / 2 - 30); - d >= c[0].x && a.drawImage(k, -d - 30, -C / 2 - 30); - C / 2 > c[0].y && a.drawImage(g, -d - 30, -C / 2 - 30);
                ...
                this.draw = function(b) {
                    if (H.loaded) {
                        ...
                        if (w) {
                            ...
                            b.fill();
                            if (w.attached) b.beginPath(), b.arc(e + 150 * a * s, h + 150 * l * s, (c + 2) * s, 0, 2 * Math.PI), b.stroke();
                            else {
                                var l = w.y + C / 2,
                                    a = (w.x + E / 2) / E,
                                    l = l / C,
                        ...
                        for (var a = 150 * s / E, l = H.objects, c = l.length, f = 0; f < c; f++) {
                            ...
                            if (1 == g.type) {
                                ...
                                var n = E / 2,
                                    ...
                                    x = g.length;
                                b.beginPath();
                                for (var v = 0; v < x; v++) e = (g[v].x + n) * a + 5 * s, h = (g[v].y + m) * a + 5 * s, 0 == v ? b.moveTo(e, h) : b.lineTo(e, h);
                                b.closePath();
                                b.fill()
                            }
                        }
            ...
            Xb = function() {
                ...
                function k(a, b) {
                    for (var c = 1;;) {
                        ...
                        switch (e) {
                            case 0:
                                (h = A[d]) ? c = h.updateNetwork(a, c, !1, b): console.log("entity with id: " + d + " not found");
                                break;
                            case 1:
                                var e = a.getUint8(c, !0),
                                    c = c + 1,
                                ...
                                switch (h) {
                                    case 5:
                                        ...
                                        break;
                                    case 4:
                                        0 == f ? k = new Ob : 1 == f ? k = new Tb : 2 == f || 3 == f ? k = new Qb(f) : 4 == f && (k =
                                                                                                                                  new Wb(f));
                                        break;
                                    case 1:
                                ...
                                }(h = k) ? (h.nick = g, h.id = d, A[d] = h, c = h.updateNetwork(a, c, !0, b)) : console.log("Unable to create entity. Entity Type is: " + e);
                                break;
                            case 2:
                                e = a.getUint16(c, !0);
                                c += 2;
                                g = a.getUint8(c);
                                c += 1;
                                (h = A[d]) ? (h.killReason = g, h.killedByID = e, d = h == w, c = h.deleteNetwork(a, c), d && K && (K = !1, w = null, J.angle = Math.PI, J.throttle = 0, B.clearTip(), Gb++, Hb++, Ja() && F.resetTutorial(),
                ...
                this.getServerAndConnect = function() {
                    ...
                    if (void 0 == b) setTimeout(m.getServerAndConnect, 200);
                        ...
                        q.ajax({
                            ...
                            type: "PUT",
                            success: function(a) {
                                if ("0" == a || "1" == a) q("#topGui").hide(), q("#topGuiConnecting").hide(), q("#roomFailed").show(), q("#tips").hide(), a = "0" == a ? "LINK EXPIRED!" : "SERVER IS FULL!", q("#linkText")[0].innerHTML = a;
                                else {
                                    a = a.split("!");
                                    m.roomID = 0;
                ...
                this.connect = function() {
                    if (R || L) {
                        ...
                        } else 0 < m.roomNumber && (a = "ws:" + a.split(":")[1] + ":" + (parseInt(m.roomNumber) +
                                                                                         8080));
                        ...
                        g.onerror = m.onError
                    } else setTimeout(m.getServerAndConnect, 100)
                };
                ...
                this.processMessage = function(a) {
                    ...
                    var e = a.getUint8(0);
                    if (0 == e) L &&
                        (a = +new Date - d, M.updateLag(a), 150 < a ? this.ping() : setTimeout(function() {
                        m.ping()
                    }, 150 - a));
                    else if (160 == e) {
                        var c = 1,
                        ...
                        Ua = rb = !0
                    } else 161 == e ? (ma = !0, q("#firstRightBox").hide(), q("#mobileBoxId").hide(), q("#afterRightBox").show(), q("#linksID").css({
                        bottom: "38px"
                    }), c = 1, g = a.getUint32(c, !0), c += 4, N = g, ba = 0, K = !0, hb()) : 180 == e || 179 ==
                        e ? (c = +new Date, g = c - h, 150 < g && L && console.log("Delta: " + g + " - LAG WARNING !"), h = c, k(a, e), objectCount = Object.keys(A).length, t.title = 0 < objectCount && L ? "brutal.io (" + qb + ", " + objectCount + ")" : "brutal.io!") : 164 == e ? b(a) : 165 == e || 181 == e ? (c = f(a, e), g = a.getUint16(c, !0), c += 2, 0 < g ? (165 == e ? (e = a.getUint16(c, !0), c += 2) : (e = a.getUint32(c, !0), c += 4), g = a.getUint16(c, !0), c += 2, B.updateRank(g, e)) : B.updateRank(0, 0), e = a.getUint32(c, !0), c += 4, a = a.getUint32(c, !0), c += 4, ab && console.log("GrabbedEnergy: " + e + ", ToBeGrabbedEnergy: " + a + " TOTAL: " +
                        (e + a))) : 166 == e && Aa.updateInfo(a)
                };
            ...
            $b = function(b) {
                ...
                k.update = function(b) {
                    ...
                    0 < ba && 1 < cameraSmooth && (cameraSmooth -= .2, 1 > cameraSmooth && (cameraSmooth =
                                                                                            1));
                    H.update(b);
                    Aa.update(b);
                    ...
                    if (null != w)
                        if (J.mouseMoved) {
                            c = 1;
                            V || (c = 2);
                            cb = (nb + (y.x * y.zoom - g.width * c / 2)) / y.zoom;
                            db = (ob + (y.y * y.zoom - g.height * c / 2)) / y.zoom;
                            mouseWorldX = cb;
                            mouseWorldY = db;
                            c = cb - w.x;
                            var d = db - w.y,
                    ...
                    l && w && K && (0 == h ? (a -= b, 0 > a && (h = 1)) : 1 == h ? (h = 2, B.showTip("CLICK TO RELEASE FLAIL FROM YOUR CAR",
                                                                                                     0)) : 3 == h ? (h = 4, B.clearTip()) : 4 != h && (5 == h ? (h = 6, B.showTip("CLICK AND HOLD TO ATTRACT FLAIL", 0, "#00FF00", "#006600")) : 7 == h && (B.clearTip(), n.localStorage.tutorial ? n.localStorage.tutorial++ : n.localStorage.tutorial = 1, l = !1)))
                };
                k.draw = function(a) {
                    if (p.loaded)
                        if (ma) {
                            if (y.setupContext(d), H.draw(d), rb) {
                                for (var b in A) {
                                    var e = A[b];
                                    e.draw(d, a)
                                }
                                for (b in A) A[b].drawAngle?.(d); // hacc
                                for (b in A) e = A[b], e.drawAfter(d, a);
                                for (b in A) A[b].drawInfo(d);
                                for (b in A) A[b].drawEnergy?.(d); // hacc
                                S.drawBehind(d);
                                S.drawLayer2(d);
                                S.draw(d);
                                S.drawExplosions(d);
                ...
                k.updateOffscreenInfo = function(a) {
                    if (null != w) {
                        var b = y.getBounds(),
                            ...
                            f = b[1].y,
                            g, l;
                        0 == a ? (g = w.flailX, l = w.flailY) : (b = da((D - ub) / $, 0, 1), Va = b * (sb - Xa) + Xa, Wa = b * (tb - Ya) + Ya, g = Va, l = Wa);
                        kingX = sb, kingY = tb;
                ...
                k.resize = function(a) {
                    t();
                    !ma && p.splashScreenLoaded && k.updateBackgroundImage();
                ...
                var t = function() {
                    ...
                    M && (M.domElement.style.position =
                          "absolute", M.domElement.style.left = .23 * g.width + "px", M.domElement.style.top = g.height - 18 - 30 + "px")
                };
                g = b;
                d = g.getContext("2d");
                k.context = d;
                ...
                p.load(function() {
                    ...
                    B = new Mb;
                    vb = new Lb;
                    m.hasConnection && !m.sentHello && m.hello();
                    setInterval(f, 40)
                });
                l = Ja();
                myName = "";
            ...
            Ea = function() {
                ...
                this.generateTintImage2 = function(a, g, c, k) {
                    ...
                    for (var p = m.getImageData(0, 0, this.width, this.height), q = p.data, s = q.length, u = 0; u < s;) q[u] = q[u++] * (1 - k) + a * k, q[u] = q[u++] * (1 - k) + g * k, q[u] = q[u++] * (1 - k) + c * k, q[u] = .8 * q[u++];
                    m.putImageData(p, 0, 0);
                    a = new Ea;
                    a.frameWithCanvas(n, e, h);
                    return a
                };
                this.generateTintImage = function(a, b, c,
                                                   d) {
                    var g = t.createElement("canvas");
                    g.width = this.width;
                    g.height = this.height;
                ...
                this.generateRGBKs = function() {
                    ...
                    e = e.getContext("2d");
                    e.drawImage(d, b, f, this.width, this.height, 0, 0, this.width, this.height);
                    for (var c = e.getImageData(0, 0, this.width, this.height).data, h = c.length, g = this.getImageCopy(), k = this.getImageCopy(), n = this.getImageCopy(), e = this.getImageCopy(), m = 0; m < h; m += 4) g.toData[m] = c[m], g.toData[m + 1] = 0, g.toData[m + 2] = 0, g.toData[m + 3] = c[m + 3], k.toData[m] = 0, k.toData[m + 1] = c[m + 1], k.toData[m + 2] = 0, k.toData[m + 3] = c[m + 3], n.toData[m] =
                        0, n.toData[m + 1] = 0, n.toData[m + 2] = c[m + 2], n.toData[m + 3] = c[m + 3], e.toData[m] = 0, e.toData[m + 1] = 0, e.toData[m + 2] = 0, e.toData[m + 3] = c[m + 3];
        ...
        gameSheetInfo = [
            ...
            ["grid_mask_hard", 104, 206, 98, 98, .5, .505],
        ...
        var Pb = function() {
            ...
            this.update = function(k) {
                ...
                for (var h = this.timeToNextJoint / 50, g = this.tailJoints.length - 1, d = 0; d <= g; d++) e = this.tailJoints[d].length, 1 >= e || (e = D - this.tailJoints[d][0].t, e > this.trailTime - this.tailAddJointInterval && (e = this.tailJoints[d][0].origY -
                    this.tailJoints[d][1].origY, this.tailJoints[d][0].x = this.tailJoints[d][1].origX + (this.tailJoints[d][0].origX - this.tailJoints[d][1].origX) * h, this.tailJoints[d][0].y = this.tailJoints[d][1].origY + e * h));
            ...
            Wb = function() {
                ...
                this.update = function(b) {
                    ...
                    if (this.beingDeleted) {
                        ...
                        g += .07 * b;
                        1 < g && delete A[this.id]
                    } else f = da((D - this.lastUpdateTime) /
                                  $, 0, 1), this.prevX = this.x, this.prevY = this.y, c = f * (this.dstY - this.origY) + this.origY, this.x = f * (this.dstX - this.origX) + this.origX, this.y = c, d += .01 * b, e += .08 * b;
                ...
                this.updateNetwork = function(b, a, d) {
                    ...
                    this.dstX = 10 * c;
                    this.dstY = 10 * b;
                    d ? (this.origX = this.dstX, this.origY = this.dstY, this.x = this.dstX, this.y = this.dstY, f = p.frames.grid_mask_hard.renderTintedFrame("#00FFFF"), k = p.frames.grid_mask_hard.renderTintedFrame("#FF0000")) :
                    this.canInterpolate = !0;
                    this.lastUpdateTime = D;
                    return a
                };
        ...
        n.onload = function() {
            L && Ib();
            null == F && (n.devicePixelRatio && (Qa = 1 < n.devicePixelRatio ? 2 : 1), F = new $b(t.getElementById("canvas")), n.addEventListener("resize",
                                                                                                                                                  F.resize, !1), J = new Kb, J.addListeners(), n.requestAnimationFrame ? n.requestAnimationFrame(bb) : setInterval(bb, 1E3 / 60), q("#overlay").show());
            F.resize();
            fb()
        }
